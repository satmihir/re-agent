...
4. **`gpt-6-luna` is offered but not the default** (B4). Resolved differently: #15 made `gpt-6-luna` the default model. The benchmark first stayed on `gpt-5.6-luna` at low effort. Since #18 it defaults to `gpt-6-luna` at high effort, which resolved 6 of 16 runs to `gpt-5.6-luna`'s 4 on the same commit (`f533070`), for about 1.75 times the cost. The two `g6-high` passes on that commit are the new baseline, and results from different models are never pooled.
